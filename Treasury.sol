...
contract TomorrowTreeTreasury {
    ...
    address TreasuryVotingContractAddress;  //Address of the Smart Contract where the community votes on treasury transfers, burns or reward / tax adjustments
...
// Only the upgradeable Treasury Voting Smart Contract can call these functions
    modifier onlyTreasuryVoter {
        require(msg.sender == TreasuryVotingContractAddress);
        _;
    }

// Only the Smart Contract responsible for voting on project proposals can call these functions
...
//Question to consider: do we make a balance mapping address => uint to keep track of the donors ?
    ...
    function sendAVAX(address payable recipient, uint amount) onlyTreasuryVoter external { 
...
//  Sets how much voters get rewarded for casting votes
    function setProjectVoterReward(uint newProjectVoterReward) onlyTreasuryVoter external {
        projectVoterReward = newProjectVoterReward;
    }

//  Rewards voters in the Smart Contract responsible for voting on Project Proposals 
